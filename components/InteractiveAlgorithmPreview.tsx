        ...
        <div className="rounded-3xl bg-white border-2 border-[#dedcff] shadow-xl shadow-[#2f27ce]/5 overflow-hidden">
            ...
            <div className="px-6 py-4 sm:px-8 sm:py-5 border-b border-[#dedcff] bg-[#dedcff]/20 flex flex-wrap items-center justify-between gap-3">
                <div className="inline-flex items-center gap-2">
                    ...
                    <span className="text-xs font-bold text-[#2f27ce] uppercase tracking-wider">
                    ...
                    {SCENARIOS.map((s) => {
                        ...
                        return (
                            <button
                                ...
                                className={`
                                    px-4 py-2 rounded-lg text-xs sm:text-sm font-bold transition-all cursor-pointer select-none leading-none
                                    ${isActive
                                        ? 'bg-[#2f27ce] text-white shadow-sm shadow-[#2f27ce]/25'
            ...
            <div className="p-6 sm:p-8 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="p-6 sm:p-7 rounded-2xl bg-[#fbfbfe] border border-[#dedcff] flex flex-col justify-between space-y-4 shadow-xs">
                        <div className="space-y-3">
                            <div className="flex items-center justify-between">
                                <span className="text-xs font-bold text-[#DC2626] uppercase tracking-wider bg-[#FEF2F2] border border-[#FECACA] px-3 py-1 rounded-full leading-none">
                                    Before Optimization
                                </span>
                            ...
                            <p className="text-base sm:text-lg text-[#050315]/80 font-medium leading-relaxed line-through decoration-[#DC2626]/50 pt-1">
                                {scenario.before.headline}
                            </p>
                        </div>

                        <div className="pt-4 border-t border-[#dedcff] space-y-2 text-sm text-[#050315]/80">
                            {scenario.before.flaws.map((flaw, idx) => (
                                <div key={idx} className="flex items-start gap-2.5 leading-snug">
                                    <span className="text-[#DC2626] font-bold text-base shrink-0 leading-none">✕</span>
                                    <span>{flaw}</span>
                                </div>
                            ))}
                        </div>
                    ...
                    <div className="p-6 sm:p-7 rounded-2xl bg-white border-2 border-[#2f27ce] shadow-lg shadow-[#2f27ce]/10 flex flex-col justify-between space-y-4 relative overflow-hidden group">
                        ...
                        <div className="space-y-3">
                            <div className="flex items-center justify-between">
                                <span className="inline-flex items-center gap-1.5 text-xs font-bold text-[#2f27ce] uppercase tracking-wider bg-[#dedcff] px-3 py-1 rounded-full leading-none shadow-2xs">
                                    <span className="w-1.5 h-1.5 rounded-full bg-[#2f27ce]" />
                                    <span>Rank #1 Optimized</span>
                                </span>
                                <span className="text-xs sm:text-sm font-bold text-[#16A34A] bg-[#F0FDF4] border border-[#BBF7D0] px-3 py-1 rounded-full leading-none shadow-2xs">
                                    Score: {scenario.after.score}/100 (+{scenario.gain} pts)
                                </span>
                            </div>

                            <p className="text-base sm:text-lg font-bold text-[#050315] leading-relaxed pt-1">
                                {scenario.after.headline}
                            </p>
                        </div>

                        <div className="pt-4 border-t border-[#dedcff] space-y-2 text-sm text-[#050315] font-medium">
                            {scenario.after.improvements.map((point, idx) => (
                                <div key={idx} className="flex items-start gap-2.5 leading-snug">
                                    <span className="text-[#2f27ce] font-bold text-base shrink-0 leading-none">✓</span>
                                    <span>{point}</span>
                                </div>
                            ))}
                        </div>
                ...
                <div className="p-4 sm:p-5 rounded-2xl bg-[#dedcff]/30 border border-[#dedcff] flex flex-col sm:flex-row items-center justify-between gap-3 text-sm">
                    <div className="flex items-center gap-2 text-[#050315]">
                        <span className="font-bold text-[#2f27ce]">Mobile Preview:</span>
                        <span className="font-medium text-[#050315]">&ldquo;{scenario.after.mobileSnippet}&rdquo;</span>
                    </div>

                    <div className="inline-flex items-center gap-1.5 font-bold text-[#2f27ce] bg-white px-3.5 py-1.5 rounded-full border border-[#dedcff] shadow-xs leading-none shrink-0">
                        <CheckIcon size={14} />
